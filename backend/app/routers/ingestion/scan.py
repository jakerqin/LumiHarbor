...
@router.post("/scan", response_model=schema.ApiResponse[dict])
def scan_and_import(
    ...
    """
    # 使用指定路径或默认 NAS 路径
    scan_path = source_path or settings.NAS_DATA_PATH

    if not os.path.exists(scan_path):
        logger.error(f"扫描路径不存在: {scan_path}")
        raise HTTPException(status_code=404, detail=f"扫描路径不存在: {scan_path}")

    logger.info(f"触发素材扫描任务 - 路径: {scan_path}, 用户: {created_by}, 可见性: {visibility}")

    def run_import():
        logger.info("后台任务开始执行: 扫描目录获取素材数据")

        # 1. 扫描目录获取素材数据
        assets_data = FilesystemScanner.scan(scan_path, created_by, visibility)
