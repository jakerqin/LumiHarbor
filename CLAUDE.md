...
- Dock：首页 · 素材 · 相册 · 笔记 · 地图 · 手机快传
- 首页：Dome Gallery（精选）、笔记时间轴；首页内嵌地球仍可能用 Mock 地点
- 素材库：瀑布流、筛选、上传、收藏、详情与相似
- 相册：列表/详情、创建、文件夹导入
- 笔记：列表/新建/详情，主编辑器为 **Novel（Tiptap）**
- 独立 `/map`：3D 地球 + Mapbox 2D，接真实足迹 API
